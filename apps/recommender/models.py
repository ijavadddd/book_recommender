...
class Recommendation:
    ...
    def rated_genre_average(user_id):
        ...
        with connection.cursor() as cursor:
            ...
            return sorted(
                genre_averages, key=lambda x: x["average_rating"], reverse=True
            )
    ...
    def suggest_genre(cls, user_id):
        ...
        order_by_clause = (
            f"ORDER BY CASE {case_statement} ELSE {len(genres) + 1} END, r.rating"
        )
    ...
    @staticmethod
    def suggest_similar_user(user_id):
        query = f"""WITH user_reviews AS (
            SELECT book_id
            FROM reviews
        ...
        SELECT b.*
        FROM other_reviews r
        JOIN similar_reviews sr ON r.user_id = sr.user_id
