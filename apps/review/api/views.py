...
class AddReviewAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            review = Review.count({"book_id": validated_data['book_id'], "user_id": request.user.id})
            if not review:
...
class UpdateReviewAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            review = Review.count({"book_id": validated_data['book_id'], "user_id": request.user.id})
            if review:
                add_review_data = Review.update({'rating': validated_data['rating']},
...
class DeleteReviewAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if review:
                delete_review_result = Review.delete({"book_id": validated_data['book_id'],
                                                      "user_id": request.user.id})
